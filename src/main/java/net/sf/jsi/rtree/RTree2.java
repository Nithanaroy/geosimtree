 ...
 * </p>
 */
public class RTree2 implements SpatialIndex, Serializable {
	...
	private TIntObjectHashMap<Node2> nodeMap = new TIntObjectHashMap<Node2>();
	...
	public void init(Properties props) {
		...
		Node2 root = new Node2(rootNodeId, 1, maxNodeEntries);
	...
	private void add(float minX, float minY, float maxX, float maxY, int id, int level) {
		...
		Node2 n = chooseNode(minX, minY, maxX, maxY, level);
		Node2 newLeaf = null;
		...
		Node2 newNode = adjustTree(n, newLeaf);

		// I4 [Grow tree taller] If node split propagation caused the root to
		// split, create a new root whose children are the two resulting nodes.
		if (newNode != null) {
			int oldRootNodeId = rootNodeId;
			Node2 oldRoot = getNode(oldRootNodeId);

			rootNodeId = getNextNodeId();
			treeHeight++;
			Node2 root = new Node2(rootNodeId, treeHeight, maxNodeEntries);
		...
		while (foundIndex == -1 && parents.size() > 0) {
			...
			if (!n.isLeaf()) {
				...
				for (int i = startIndex; i < n.entryCount; i++) {
					if (Rectangle.contains(n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i], r.minX, r.minY, r.maxX,
							r.maxY)) {
	...
	public void nearest(Point p, TIntProcedure v, float furthestDistance) {
		Node2 rootNode = getNode(rootNodeId);
	...
	private void createNearestNDistanceQueue(Point p, int count, PriorityQueue distanceQueue, float furthestDistance) {
		...
		while (parents.size() > 0) {
			...
			if (!n.isLeaf()) {
				...
				boolean near = false;
				for (int i = startIndex; i < n.entryCount; i++) {
					if (Rectangle.distanceSq(n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i], p.x,
				...
				for (int i = 0; i < n.entryCount; i++) {
					float entryDistanceSq = Rectangle.distanceSq(n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i],
							p.x, p.y);
					int entryId = n.ids[i];
	 ...
	 */
	private Node2 splitNode(Node2 n, float newRectMinX, float newRectMinY, float newRectMaxX, float newRectMaxY, int newId) {
		...
		Node2 newNode = null;
		newNode = new Node2(getNextNodeId(), n.level, maxNodeEntries);
		...
		if (log.isDebugEnabled()) {
			...
			log.debug("Node2 " + n.nodeId + " split. New area increased by " + percentageIncrease + "%");
		}
	 ...
	 */
	private void pickSeeds(Node2 n, float newRectMinX, float newRectMinY, float newRectMaxX, float newRectMaxY, int newId, Node2 newNode) {
		...
		for (int i = 0; i < n.entryCount; i++) {
			...
			if (log.isDebugEnabled()) {
				log.debug("Entry " + i + ", dimension X: HighestLow = " + tempHighestLow + " (index " + tempHighestLowIndex + ")"
						+ ", LowestHigh = " + tempLowestHigh + " (index " + tempLowestHighIndex + ", NormalizedSeparation = "
		...
		for (int i = 0; i < n.entryCount; i++) {
			...
			if (log.isDebugEnabled()) {
				log.debug("Entry " + i + ", dimension Y: HighestLow = " + tempHighestLow + " (index " + tempHighestLowIndex + ")"
						+ ", LowestHigh = " + tempLowestHigh + " (index " + tempLowestHighIndex + ", NormalizedSeparation = "
						+ normalizedSeparation);
			}
		...
		if (highestLowIndex == -1) {
			...
			newNode.addEntry(n.entriesMinX[highestLowIndex], n.entriesMinY[highestLowIndex], n.entriesMaxX[highestLowIndex],
					n.entriesMaxY[highestLowIndex], n.ids[highestLowIndex]);
	 ...
	 */
	private int pickNext(Node2 n, Node2 newNode) {
		...
		for (int i = 0; i < maxNodeEntries; i++) {
			if (entryStatus[i] == ENTRY_STATUS_UNASSIGNED) {
				...
				if (n.ids[i] == -1) {
					log.error("Error: Node2 " + n.nodeId + ", entry " + i + " is null");
				}

				float nIncrease = Rectangle.enlargement(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY, n.entriesMinX[i], n.entriesMinY[i],
				...
				if (difference > maxDifference) {
					...
					if (nIncrease < newNodeIncrease) {
						...
						nextGroup = 1;
					} else if (Rectangle.area(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY) < Rectangle.area(newNode.mbrMinX, newNode.mbrMinY,
							newNode.mbrMaxX, newNode.mbrMaxY)) {
						nextGroup = 0;
					} else if (Rectangle.area(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMaxX, newNode.mbrMaxY) < Rectangle
				...
				if (log.isDebugEnabled()) {
					log.debug("Entry " + i + " group0 increase = " + nIncrease + ", group1 increase = " + newNodeIncrease + ", diff = "
	 ...
	 */
	private float nearest(Point p, Node2 n, float furthestDistanceSq, TIntArrayList nearestIds) {
		for (int i = 0; i < n.entryCount; i++) {
	 ...
	 */
	private boolean intersects(Rectangle r, TIntProcedure v, Node2 n) {
		for (int i = 0; i < n.entryCount; i++) {
			if (Rectangle.intersects(r.minX, r.minY, r.maxX, r.maxY, n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i],
					n.entriesMaxY[i])) {
				if (n.isLeaf()) {
					...
					Node2 childNode = getNode(n.ids[i]);
	 ...
	 */
	private void condenseTree(Node2 l) {
		...
		while (eliminatedNodeIds.size() > 0) {
			Node2 e = getNode(eliminatedNodeIds.pop());
	 ...
	 */
	private Node2 chooseNode(float minX, float minY, float maxX, float maxY, int level) {
		// CL1 [Initialize] Set N to be the root node
		Node2 n = getNode(rootNodeId);
		parents.clear();
		parentsEntry.clear();
		...
		while (true) {
			...
			float leastEnlargement = Rectangle.enlargement(n.entriesMinX[0], n.entriesMinY[0], n.entriesMaxX[0], n.entriesMaxY[0], minX,
					minY, maxX, maxY);
			...
			for (int i = 1; i < n.entryCount; i++) {
				...
				if ((tempEnlargement < leastEnlargement)
						|| ((tempEnlargement == leastEnlargement) && (Rectangle.area(tempMinX, tempMinY, tempMaxX, tempMaxY) < Rectangle
								.area(n.entriesMinX[index], n.entriesMinY[index], n.entriesMaxX[index], n.entriesMaxY[index])))) {
					index = i;
	 ...
	 */
	private Node2 adjustTree(Node2 n, Node2 nn) {
		...
		while (n.level != treeHeight) {
			...
			Node2 parent = getNode(parents.pop());
	...
	private boolean checkConsistency(int nodeId, int expectedLevel, Rectangle expectedMBR) {
		...
		if (n.level != expectedLevel) {
			log.error("Error: Node2 " + nodeId + ", expected level " + expectedLevel + ", actual level " + n.level);
			return false;
		}
		...
		if (!actualMBR.equals(calculatedMBR)) {
			log.error("Error: Node2 " + nodeId + ", calculated MBR does not equal stored MBR");
		...
		if (expectedMBR != null && !actualMBR.equals(expectedMBR)) {
			log.error("Error: Node2 " + nodeId + ", expected MBR (from parent) does not equal stored MBR");
			return false;
		}
		...
		if (expectedMBR != null && actualMBR.sameObject(expectedMBR)) {
			log.error("Error: Node2 " + nodeId + " MBR using same rectangle object as parent's entry");
			return false;
		}

		for (int i = 0; i < n.entryCount; i++) {
			if (n.ids[i] == -1) {
				log.error("Error: Node2 " + nodeId + ", Entry " + i + " is null");
				return false;
			}
	 ...
	 */
	private Rectangle calculateMBR(Node2 n) {
